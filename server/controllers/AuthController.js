 // In this file we have all the API End points
...
export const getUserInfo = async (request, response, next) => {
    try {
        ...
        if (!userData) {
            return response.status(404).send("User does not exist with the given id.");
        }
...
export const removeProfileImage = async (request, response, next) => {
    try {
        ...
        return response.status(200).send("Profile image is removed.")
